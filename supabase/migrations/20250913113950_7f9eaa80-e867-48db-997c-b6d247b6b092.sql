...
CREATE INDEX IF NOT EXISTS idx_notes_content_search ON public.notes USING GIN(to_tsvector('english', title || ' ' || content));
CREATE INDEX IF NOT EXISTS idx_folders_user_name ON public.folders(user_id, name);
CREATE INDEX IF NOT EXISTS idx_flashcards_note ON public.flashcards(note_id, user_id);